...
Open a file with the given filename in given mode. "r" for read, "w" for write, etc.
```c
import std::io;
...
fn void! main() 
{
    ...
    File! file = file::open("hello.txt", "r");
    
    // Handle errors, e.g. FILE_NOT_FOUND
    if (catch excuse = file) 
    {
        io::printfn("Open failed: %s", excuse);
        return;
    }
}
```
...
Read content from a file into a buffer. Return amount of bytes read.
Based on the libc function.
```c 
import std::io;
...
fn void! main() 
{
    ...
    // If the read fails set size to 0 
    usz size = file.read(&buf) ?? 0;
    
    if (size > 0) {
        // Output as string
        io::printfn("%s", (String)&buf);
    }
}
``` 

### usz! File.write(&self, char[] buffer)
Write to a file based on a character input buffer. 
Based on the libc function.
```c
import std::io;
...
fn void! main() 
{
    ...
    // If file write fails, crashes with error
    // See optionals for additional examples on handling
    usz size = file.write("Score: 50\n")!!;
    io::printfn("Wrote to file!");
}
```

:::Note
If the file is not already created you may have to use a mode such as w+
to ensure that it is also made.
:::
...
fn void! main() 
{
    ...
    String isfile = file::is_file("scoreboard.txt"); 

    // Doesn't exist in current directory
    String isnotfile = file::is_file("ellaminnowpea");
    
    io::printn(isfile); // true
    io::printn(isnotfile); // false
}
```

### File! open_path(Path path, String mode)
Open a file pointed to by a Path struct, with the given mode. 
Allows for easy handling of folders and multiple files.
```c
import std::io;

fn void! main() 
{
    String[*] files = {"app.c3", "db.c3", "route.c3"}; 
    Path base_path, path;
    char[2048] buf;

    // Add default path as backend/
    base_path = path::new("backend")!!;

    // explore the backend folder 
    // based on the defined files
    foreach (current : files) 
    {
        path = base_path.new_append(current)!!;
        File! file = file::open_path(path, "r");

        if (catch excuse = file)
        {
            io::printfn("%s : %s", excuse, path);
            continue;
        }
...
fn void! main() 
{
    ...
    // Get size from file
    size = file::get_size("hello.txt")!!; 
    io::printn(size);
    
    // Get size from path
    path = path::new("./hello.txt")!!;
    size = file::get_size(path.basename())!!;
    io::printn(size);
}
```
